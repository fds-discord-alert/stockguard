...
2.  **환경변수 설정 (`.env`)**:
    프로젝트 루트 디렉토리에 `.env` 파일을 생성하고 아래와 같이 본인의 디스코드 웹후크 URL을 입력합니다.
    ```bash
...
```
**예상 결과**: `BLOCKED` 응답 및 Discord에 **USA** 접속 알림 전송.
...
## 🔔 4단계: Discord 알림 (Embed 적용)
팀원의 로직 통합으로 단순 텍스트가 아닌 **상세 임베드(Embed) 메시지**가 전송됩니다.
- **심각도별 색상**: 빨강(Critical), 노랑(Warning), 파랑(High)으로 자동 구분.
- **상세 정보**: 사용자 ID, 종목명, 금액, 접속 IP, 탐지 사유 포함.

---

## 🐳 5단계: Docker 컨테이너 실행
```bash
# Docker Compose를 이용한 원클릭 실행 (앱 + Redis)
docker-compose up -d
```
> 주의: Docker 환경에서 알림을 받으려면 `docker-compose.yaml`에 `webhook_url` 환경변수를 추가하거나 `.env` 파일을 함께 마운트해야 합니다.
